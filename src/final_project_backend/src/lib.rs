...
#[ic_cdk::query]
fn get_proposal_status(key: u64) -> Option<String> {
    PROPOSAL_MAP.with(|p| {
        let proposal_opt: Option<Proposal> = p.borrow().get(&key);
        ...
        match proposal_opt {
            Some(proposal) => {
                ...
                if proposal.voted.len() < 5 {
                    return Some(String::from("Undecided"));
                }

                // Calculate the total votes
                let total_votes = proposal.approve + proposal.reject + proposal.pass;

                // Determine the majority condition (at least 50%)
                let majority_condition = total_votes / 2;

                // Check the status based on votes
                if proposal.approve >= majority_condition {
                    ...
                    Some(String::from("Undecided"))
                }
            }
            None => None, // Proposal not found
        }
    })
}